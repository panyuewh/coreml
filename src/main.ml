...
let rec eval (env : env) (e : expr) : value = match e with
  ...
  | Left e -> VLeft (eval env e)
  | Right e -> VRight (eval env e)
...
(** [eval_fst env e] is the [v] such that
    [<env, fst e> ==> v]. *)
and eval_fst env e = match eval env e with
  | VPair (v, _) -> v
  | _ -> failwith pair_guard_err

(** [eval_snd env e] is the [v] such that
    [<env, snd e)> ==> v]. *)
and eval_snd env e =  match eval env e with
  | VPair (_, v) -> v
  | _ -> failwith pair_guard_err

(** [eval_match env e x1 e1 x2 e2] is the [v] such that
    [<env, match e with Left x1 -> e1 | Right x2 -> e2> ==> v]. *)
and eval_match env e x1 e1 x2 e2 = match eval env e with
  | VLeft v -> 
      let env' = Env.add x1 v env in 
      eval env' e1 
  | VRight v -> 
      let env' = Env.add x2 v env in 
      eval env' e2 
  | _ -> failwith match_guard_err
